...
cCommandGroup* cAnimationControl::pCreateCommandGroup(int unID)
{
	cCommandGroup* pNC = new cCommandGroup(unID);
...
void cAnimationControl::AddCommandGroup(cCommandGroup* pCommandGroup)
{
	...
	else
	{
		...
		if (pCommandGroup->GetUniqueID() != 0)
		{
			for (std::vector<cCommandGroup*>::iterator itCG = this->vec_pCommandGroup.begin();
				itCG != this->vec_pCommandGroup.end(); itCG++)
			{
				if (pCommandGroup->GetUniqueID() == (*itCG)->GetUniqueID())
				{
					// Don't add the command group
					return;
				}
			}
		}
